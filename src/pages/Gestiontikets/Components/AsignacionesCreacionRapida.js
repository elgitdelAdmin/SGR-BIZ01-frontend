...
const AsignacionesCreacionRapida = ({
  formik,
  permisosActual,
  ...
  const obtenerOpcionesSubfrente = (currentIdSubFrente) => {
    if (esCreacionRapida) {
      ...
      const idConsultor = formik.values.asignaciones[editingIndex]?.IdConsultor;
      if (!idConsultor) return [];

      let allowedFrentes = null;
      const selectedConsultor = (consultores || []).find(c => Number(c.id) === Number(idConsultor));
      if (selectedConsultor && (selectedConsultor.especializaciones || selectedConsultor.Especializaciones)) {
        const espList = selectedConsultor.especializaciones || selectedConsultor.Especializaciones;
        allowedFrentes = new Set(espList.map(esp => Number(esp.idFrente || esp.IdFrente)));
      }

      const todosSubfrentes = [];
      (frentes || []).forEach((f) => {
        if (!allowedFrentes || allowedFrentes.has(Number(f.id))) {
          (f.subFrente || []).forEach((sf) => {
            todosSubfrentes.push({
              idSubFrente: sf.id,
              nombre: sf.nombre,
              idFrente: f.id,
            });
  ...
  const sePuedeAgregar = esCreacionRapida ? true : opcionesDisponibles.length > 0;
  ...
  return (
    <div className="field col-12 md:col-12">
        ...
        >
          ...
          <Column
            ...
            body={(asignacion) => {
              const originalIndex = formik.values.asignaciones.findIndex((a) => a.idUnico === asignacion.idUnico);
              if (originalIndex === -1) return "—";
              const name = (consultores || []).find((c) => Number(c.id) === Number(asignacion.IdConsultor))?.nombre || "—";
      ...
      {/* MODAL DE EDICION */}
      <Dialog
        header={esCreacionRapida ? "Crear Asignación" : (editingIndex !== null && formik.values.asignaciones[editingIndex]?.IdSubFrente ? "Editar Asignación" : "Agregar Asignación")}
            ...
            <Boton
              ...
              onClick={() => {
                if (editingIndex !== null) {
                  ...
                  // formik.handleSubmit(); // No enviamos al servidor, solo guardamos en memoria
                }
                setVisibleModal(false);
                setEditingIndex(null);
              }}
      ...
      >
        {editingIndex !== null && formik.values.asignaciones[editingIndex] && (
          <div className="p-fluid grid">
            {/* Subfrente */}
            <div className="field col-12 md:col-6" style={{ order: esCreacionRapida ? 2 : 1 }}>
              ...
              <DropdownDefault
                ...
                onChange={async (e) => {
                  ...
                  let idFrente = null;
                  if (esCreacionRapida) {
                    const parentFrente = (frentes || []).find((f) =>
                      (f.subFrente || []).some((sf) => Number(sf.id) === Number(idSubFrente))
                    );
                    idFrente = parentFrente ? parentFrente.id : null;
                  } else {
                    const seleccionado = subfrentesSeleccionados.find((s) => s.idSubFrente === idSubFrente);
                    idFrente = seleccionado?.idFrente;
                  }
                  ...
                  if (placeholderIdx !== -1) {
                    ...
                    if (!esCreacionRapida) {
                      const especializacion = (formik.values.frenteSubFrentes || []).find(
                        (esp) => esp.activo !== false && Number(esp.idSubFrente) === Number(idSubFrente)
                      );
                      if (especializacion) {
                        const asig = formik.values.asignaciones[editingIndex];
                        if (!asig.FechaAsignacion && especializacion.fechaInicio) {
                          formik.setFieldValue(`asignaciones[${editingIndex}].FechaAsignacion`, especializacion.fechaInicio);
                        }
                        if (!asig.FechaDesasignacion && especializacion.fechaFin) {
                          formik.setFieldValue(`asignaciones[${editingIndex}].FechaDesasignacion`, especializacion.fechaFin);
                        }
                      }
                    } else {
            ...
            {/* Consultor */}
            <div className="field col-12 md:col-6" style={{ order: esCreacionRapida ? 1 : 2 }}>
              ...
              <DropdownDefault
                ...
                optionValue="id"
                onChange={(e) => {
                  const idConsultor = e.value;
                  const oldIdSubFrente = formik.values.asignaciones[editingIndex].IdSubFrente;
                  formik.setFieldValue(`asignaciones[${editingIndex}].IdConsultor`, idConsultor);

                  if (esCreacionRapida && idConsultor) {
                    const selectedConsultor = (consultores || []).find(c => Number(c.id) === Number(idConsultor));
                    const firstEsp = selectedConsultor?.especializaciones?.[0] || selectedConsultor?.Especializaciones?.[0];

                    if (firstEsp) {
                      const idSubFrente = Number(firstEsp.idSubFrente || firstEsp.IdSubFrente);
                      const idFrente = Number(firstEsp.idFrente || firstEsp.IdFrente);
                      ...
                      if (!asig.FechaAsignacion) {
                        formik.setFieldValue(`asignaciones[${editingIndex}].FechaAsignacion`, fechaReferencia);
                      }
                      if (!asig.FechaDesasignacion) {
                        formik.setFieldValue(`asignaciones[${editingIndex}].FechaDesasignacion`, fechaReferencia);
                      }

                      // Check if already in frenteSubFrentes
                      const existeEsp = (formik.values.frenteSubFrentes || []).some(
                        (esp) => esp.activo !== false && Number(esp.idSubFrente) === idSubFrente
                      );
                      ...
                      // Si había un subfrente anterior autogenerado y ya no se usa, lo quitamos
...
export default AsignacionesCreacionRapida;
