...
const EspecializacionesCreacionRapida = ({
  formik,
  frentes,
  permisosActual,
  ...
  const getAsignacionVinculada = (rowData) => {
    return (formik.values.asignaciones || []).find(
      (a) => a.Activo !== false && (
        (a._frenteSubFrenteUid && a._frenteSubFrenteUid === rowData._uid) ||
        (!a._frenteSubFrenteUid && Number(a.IdSubFrente) === Number(rowData.idSubFrente))
      )
    );
  };
    ...
    <>
      ...
      <div className="field col-12" style={{ marginTop: 8 }}>
        {!frentes || frentes.length === 0 ? (
          ...
          <>
            <div id="tabla-especializaciones" className={`overflow-x-auto w-full table-warning-wrapper ${highlightWarning ? "table-warning-blink" : ""}`}>
              <DatatableDefault
                showSearch={false}
                paginator={false}
...
export default EspecializacionesCreacionRapida;
